...
// navigation chain: property references, method calls, projections, selections, etc.
  ...
  * @param stream the InputStream to parse.
  ...
  private Token jj_consume_token(int kind) throws ParseException {
    ...
    // ---- Logic not generated by JavaCC/JJTree.  If the OGNL grammar specification changes, this logic must be re-added manually. ----
    if (kind == IDENT  && token.kind != IDENT) {
      // Expected kind is IDENT (identifier), so we may be able to process certain tokens as identifiers, under limited circumstances.
      // Note: This processing is limited to cases where such tokens are part of a compound package/class name ("a.b.c.d") and the tokens
      //       cannot be the first element (so it cannot be the "a" in "a.b").  It appears the OGNL grammar cannot recognize a starting
      //       token as anything but that token, so only in cases where it is already processing a token string it considers to be
      //       an identifier can we possibly override.
      if (oldToken != null && oldToken.kind == 43) {
        // The previous token was ".", the only token that might resonably precede a possible "package name token" (or in some cases
        // "class name token").  If the next token is one of the possible package name strings, then we will override its type to match
        // the expected kind of IDENT, as it can be a valid identifier too (under these strict circumstances).
        // See the OgnlParserTokenManager.jjstrLiteralImages array for the possible matches (includes tokens like "and", "or", and so forth).
        if (tokenKindValidForPackageName(token)) {
          token.kind = IDENT;  // Override the parsed Token kind to be IDENT (previous token ".", current token is a valid identifier too).
        }
      }
    }
    // ---- Logic not generated by JavaCC/JJTree.  If the OGNL grammar specification changes, this logic must be re-added manually. ----
  ...
  // ---- Logic not generated by JavaCC/JJTree.  If the OGNL grammar specification changes, this logic must be re-added manually. ----
  /**
   * Helper method to test if a {@link Token} is of a kind that may be a valid package name in the appropriate circumstances.
   * The decision is based on the string literals for tokens, as per {@link OgnlParserTokenManager#jjstrLiteralImages} where the
   * token kinds match the index of that array.
   * 
   * @param token the token to test its kind against possible package name validity.
   * @return true if the token kind is valid when its literal text is considered as a package name element, false otherwise.
   */
  private boolean tokenKindValidForPackageName(Token token) {
    boolean result;
    if (token == null) {
      return false;
    }
    switch(token.kind) {
      // Token kinds that can be valid package names (true result).
      case 6:
      case 8:
      case 10:
      case 12:
      ...
      case 48:
        ...
        break;
      default:
        // All other token kinds (false result).
        result = false;
        break;
    }
    return result;
